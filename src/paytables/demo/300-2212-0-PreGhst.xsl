...
function scenarioConvertor(scenario) {
	...
	if(scenarioDelimits[2] !== undefined){
		bonusDetails =  scenarioDelimits[2].split(',');
	}
	return {
...
function getWinEmotiNums(spinEmotiPosition, spinFeature, totalWinMap, mutiplierMap){
    ...
    var winMap = {
        'L': 0,'A': 0,'B': 0,'C': 0,'D': 0,'E': 0,'F': 0
    };
    totalWinDistance.forEach(function(value){
        winMap[value]++;
    });
    var mutiplier = 0;
...
function doFormatJson(scenario, tranMap, prizeMap) {
	...
	for(var i = 0; i < result.wheelDetails.length; i++){
        ...
        r.push('<tr>');
        r.push('<td colspan="9" style="font-weight: 600">' + tranMap.spin + ' ' + (i+1) + '</td>');
        r.push('</tr>');
        //for emoti collect 
		r.push('<tr>');
		r.push('<td class="tablehead" colspan="2" style="padding-right:10px">' + tranMap.emotiWheel + '</td>');
        for(var j = 0; j < emotiWheelLabels.length; j++){
            r.push('<td class="tablehead" style="padding-right:10px">' + emotiWheelLabels[j] + '</td>');
        }
		r.push('</tr>');
        ...
        r.push('<tr>');
		r.push('<td colspan="2" style="padding-right:10px">' + tranMap.collected + '</td>');
        ...
        //for mutiplier
        r.push('<tr>');
		r.push('<td class="tablehead" colspan="2" style="padding-right:10px">' + tranMap.mutiplierWheel + '</td>');
        for(var j = 0; j < mutiplierLables.length; j++){
            r.push('<td class="tablehead" style="padding-right:10px">' + mutiplierLables[j] + '</td>');
        }
        r.push('</tr>');
        ...
        //for feature
        r.push('<tr>');
		r.push('<td class="tablehead" colspan="2" style="padding-right:10px">' + tranMap.featureWheel + '</td>');
        for(var j = 0; j < featureTranMap.length; j++){
            r.push('<td class="tablehead" style="padding-right:10px">' + featureTranMap[j] + '</td>');
        }
        r.push('</tr>');
        ...
        r.push('<tr>');
		r.push('<td colspan="2" style="padding-right:10px">' + tranMap.prizes + '</td>');
        for(var j = 0; j < featureLables.length; j++){
            r.push('<td style="padding-right:10px">');
        ...
        r.push('<tr>');
		r.push('<td class="tablehead" colspan="2" style="padding-right:10px">' + tranMap.emoticons + '</td>');
        for(var j = 0; j < emotiWheelLabels.length; j++){
            r.push('<td class="tablehead" style="padding-right:10px">' + emotiWheelLabels[j] + '</td>');
        }
        r.push('</tr>');
        ...
        r.push('<tr>');
		r.push('<td colspan="2" style="padding-right:10px">' + tranMap.cumulative + '</td>');
        var k = 0;
        for (var key in totalWinMap) {
            var element = totalWinMap[key];
            r.push('<td style="padding-right:10px">' + element +' / '+ wheelsDescribed.winWheel[k] + '</td>');
            k++;
        }
        r.push('</tr>');
        ...
        for (var key in totalWinMap) {
            ...
            if(key !== 'L'){
                if(element === wheelsDescribed.winWheel[m]){
                    r.push('<td style="padding-right:10px">' + prizeMap[key] + '</td>');
                }else{
                    r.push('<td style="padding-right:10px">' + '</td>');
                }
            }else{
                r.push('<td>' + '</td>');
        ...
        if(currWinBonus){
            r.push('<tr>');
            r.push('<td class="tablehead" colspan="2" style="padding-right:10px">' + tranMap.bonusGame + '</td>');
            if(result.bonusDetails.length > 7){
                for (var n = 0; n < 7; n++) {
                    r.push('<td style="padding-right:10px">' + tranMap.spin+ ' ' +(n+1) + '</td>');
                }
                r.push('</tr>'); 
                ...
                r.push('<tr>');
                r.push('<td colspan="2" style="padding-right:10px">' + '</td>');
